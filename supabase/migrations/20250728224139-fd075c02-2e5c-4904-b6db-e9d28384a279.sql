...
-- Enable RLS on the most critical remaining tables and add necessary policies

-- Enable RLS on all major tables that don't have it
ALTER TABLE public.active_user_count ENABLE ROW LEVEL SECURITY;
...
-- Add missing critical policies for active_user_count
CREATE POLICY "Service role can manage user count" 
ON public.active_user_count 
FOR ALL 
