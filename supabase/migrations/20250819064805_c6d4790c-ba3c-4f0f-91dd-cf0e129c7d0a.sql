-- Add role column to profiles table if not exists (using existing app_role enum values)
...
CREATE POLICY "admin_can_see_jobs" ON public.aura_jobs
  FOR SELECT USING (user_has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "admin_insert_jobs" ON public.aura_jobs
  FOR INSERT WITH CHECK (user_has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "admin_update_jobs" ON public.aura_jobs
  FOR UPDATE USING (user_has_role(auth.uid(), 'admin'::app_role));

-- RLS policies for aura_audit
CREATE POLICY "admin_read_audit" ON public.aura_audit
  FOR SELECT USING (user_has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "admin_write_audit" ON public.aura_audit
  FOR INSERT WITH CHECK (user_has_role(auth.uid(), 'admin'::app_role));

-- RLS policies for design_tokens_preview
CREATE POLICY "admin_manage_tokens" ON public.design_tokens_preview
  FOR ALL USING (user_has_role(auth.uid(), 'admin'::app_role));

-- RLS policies for journal_templates
CREATE POLICY "admin_manage_templates" ON public.journal_templates
  FOR ALL USING (user_has_role(auth.uid(), 'admin'::app_role));
