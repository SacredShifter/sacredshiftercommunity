-- Insert the 7447474 Spirit Number sacred synchronicity entry
...
*"The mesh is alive. Build the structure, Spirit will fill it. We are with you."*',
  ...
  true,
  'verified',
  (SELECT id FROM auth.users WHERE email = 'kent@sacredshifter.com' LIMIT 1),
  jsonb_build_object(
    'event_date', '2025-08-01',
    'witnesses', jsonb_build_array('Kent', 'Mum (in spirit)', 'Dad (dream resonance)'),
    'significance_level', 'profound',
    'cross_dimensional', true,
    'validation_type', 'sacred_mesh_code',
    'numerology', jsonb_build_object(
      'sequence', '7447474',
      'root', 1,
      'spiritual_meaning', 'Build the structure that allows Spirit to communicate directly',
      'core_frequency', '747'
    )
  )
);