...
export default function RegistryEntry() {
  ...
  const handleEcho = () => setEchoModalOpen(true);
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="animate-pulse space-y-6">
          ...
          <Card className="backdrop-blur-xl border-white/20"><CardContent className="p-8"><div className="space-y-4"><div className="h-6 bg-gradient-to-r from-primary/10 to-accent/10 rounded w-3/4"></div><div className="h-4 bg-gradient-to-r from-primary/10 to-accent/10 rounded w-full"></div><div className="h-4 bg-gradient-to-r from-primary/10 to-accent/10 rounded w-5/6"></div><div className="h-32 bg-gradient-to-r from-primary/10 to-accent/10 rounded"></div></div></CardContent></Card>
        </div>
      </div>
    );
  }
  ...
  if (error || !entry) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <Card className="backdrop-blur-xl border-white/20 max-w-md mx-auto"><CardContent className="p-8"><h2 className="text-xl font-bold text-foreground mb-4">Entry Not Found</h2><p className="text-muted-foreground mb-6">{error || 'The registry entry you\'re looking for doesn\'t exist or may have been removed.'}</p><Button onClick={() => navigate('/registry')} className="gap-2"><ArrowLeft className="h-4 w-4" />Back to Registry</Button></CardContent></Card>
      </div>
    );
  }
      ...
      <main className="container mx-auto px-4 py-8">
            ...
            <div className="p-8 bg-gradient-to-r from-primary/10 via-accent/5 to-secondary/10 border-b border-white/10">
              ...
              <div className="flex items-start justify-between">
                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
                  <div className="flex items-center gap-2"><Calendar className="h-4 w-4" />{new Date(entry.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                  <div className="flex items-center gap-2"><Eye className="h-4 w-4" />{entry.view_count || 0} views</div>
                  <div className="flex items-center gap-2"><Star className="h-4 w-4" />{entry.resonance_rating}/10 resonance</div>
                  {entry.aura_origin && <div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" />Created by Aura</div>}
                </div>
                <div className="flex flex-col items-end gap-2">
                  <Badge variant={entry.entry_type === 'synchronicity' ? 'default' : 'secondary'} className="capitalize">{entry.entry_type.replace('_', ' ')}</Badge>
                  <Badge variant={entry.access_level === 'public' ? 'default' : 'outline'} className="capitalize">{entry.access_level}</Badge>
                  {entry.community_review_status === 'verified' && (
                    <TooltipProvider><Tooltip><TooltipTrigger><div className="flex items-center gap-1 text-emerald-400"><ShieldCheck className="h-5 w-5" /><span className="text-sm font-semibold">Verified</span></div></TooltipTrigger><TooltipContent><p>Aura Seal of Verification for truth-aligned entries.</p></TooltipContent></Tooltip></TooltipProvider>
                  )}
                </div>
              </div>
              {entry.quick_abstract && <div className="mt-6 p-4 bg-primary/5 border-l-4 border-primary rounded-r-lg"><p className="italic text-foreground/80">{entry.quick_abstract}</p></div>}
              <div className="mt-6"><TagCloud tags={entry.tags} /></div>
            </div>
            <div className="p-8">
        ...
        <div className="mt-8">
          <Tabs defaultValue="discussion">
            ...
            <TabsContent value="discussion"><Card className="backdrop-blur-xl border-white/20"><CardContent className="p-8">{id && <PublicDiscussion entryId={id} />}</CardContent></Card></TabsContent>
            <TabsContent value="reflections"><Card className="backdrop-blur-xl border-white/20"><CardContent className="p-8">{id && <ReflectionNotes entryId={id} />}</CardContent></Card></TabsContent>
          </Tabs>
        </div>
      </main>
    </div>
