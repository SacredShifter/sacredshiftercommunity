...
export function useRegistryOfResonance() {
  ...
  const fetchEntries = useCallback(async (viewMode: 'my' | 'collective' | 'drafts' = 'my') => {
    console.log('fetchEntries called with viewMode:', viewMode);
    ...
    try {
      ...
      switch (viewMode) {
        case 'my':
          console.log('Fetching my entries for user ID:', user.id);
          query = query.eq('user_id', user.id);
          break;
        case 'collective':
          console.log('Fetching collective entries (public and verified)');
          query = query.eq('access_level', 'Public').eq('is_verified', true);
          break;
        case 'drafts':
          console.log('Fetching draft entries for user ID:', user.id);
      ...
      console.log(`Fetched ${data?.length || 0} entries:`, data);
  ...
  const createEntry = useCallback(async (entryData: NewRegistryEntry) => {
    ...
    try {
      console.log('Creating entry with data:', { ...entryData, user_id: user.id });
  ...
  const togglePin = useCallback(async (id: string) => {
    try {
      ...
      await updateEntry(id, { is_pinned: !entry.is_pinned });
    } catch (err) {
  ...
  useEffect(() => {
    console.log('useRegistryOfResonance useEffect triggered, user:', user);
    if (user) {
      fetchEntries();
    }
  }, [fetchEntries, user]);
  ...
  // Comments functions
  const getComments = useCallback(async (entryId: string): Promise<Comment[]> => {
    try {
      const { data, error } = await supabase
